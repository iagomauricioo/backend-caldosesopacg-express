...
export class ProdutoRepositoryDatabase implements ProdutoRepository {
  ...
  async buscarProdutos(): Promise<Produto[]> {
    const query = `SELECT  p.id, p.nome, p.descricao, p.disponivel, p.ordem_exibicao, json_agg(json_build_object( 'tamanho_ml', v.tamanho_ml, 'nome_tamanho', v.nome_tamanho, 'preco_centavos', v.preco_centavos )) AS variacoes FROM produtos p JOIN variacoes_produto v ON v.produto_id = p.id GROUP BY p.id ORDER BY p.ordem_exibicao`;
    ...
    return produtosData.map((produtoData: any) => {
      ...
      return new Produto(produtoData.id, produtoData.nome, produtoData.descricao, produtoData.disponivel, produtoData.ordem_exibicao, variacoes);
    });
  }
